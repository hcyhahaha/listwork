...
class False extends React.Component {
    ...
    render() {
        return (<div className={appStyle.content} style={{ display: this.state.mystore.listwork[this.props.index].isFinish ? "none" : "block" }}>
            <div>
                <button
                    style={{
                        outline: "none",
                    ...
                    className={appStyle.btn} onClick={(e) => { this.isFinish(e, this.state.mystore.listwork[this.props.index].id) }}>{this.state.mystore.listwork[this.props.index].isFinish}</button>
                <div
                    style={{
                        fontFamily: "幼圆",
                        paddingLeft: "20px",
    ...
    componentDidMount() {
        ...
        this.isFinish = function (e, id) {
            axios.get("http://10.55.58.252:7001/updatefalse", { params: { id: id } })
                .then((res) => {
